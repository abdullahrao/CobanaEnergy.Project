...
namespace CobanaEnergy.Project.Controllers.Accounts.ClawbacksDashboard
{
    [Authorize(Roles = "Accounts,Controls")]
    public class ClawbacksDashboardController : BaseController
    {
        private readonly ApplicationDBContext db;
        private readonly List<string> clawbackStatuses = new List<string>
        {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            var model = new ClawbacksDashboardViewModel
            {
                Suppliers = suppliers
            };

            return View("~/Views/Accounts/ClawbacksDashboard/ClawbacksDashboard.cshtml", model);
        }

        [HttpPost]
        [ValidateJsonAntiForgeryToken]
        public async Task<JsonResult> GetClawbackContracts(int? supplierId, DateTime? startDate, DateTime? endDate)
        {
            try
            {
                ...
                var electricContracts = electricContractsList
                    .Select(e => new
                    {
                        ...
                        Status = db.CE_ContractStatuses
                            .FirstOrDefault(cs => cs.EId == e.EId && cs.Type == "Electric" && clawbackStatuses.Contains(cs.PaymentStatus)),
                ...
                var gasContracts = gasContractsList
                    .Select(g => new
                    {
                        Contract = g,
                        Status = db.CE_ContractStatuses
                            .FirstOrDefault(cs => cs.EId == g.EId && cs.Type == "Gas" && clawbackStatuses.Contains(cs.PaymentStatus)),
                        Reconciliation = db.CE_CommissionAndReconciliation
                ...
                var contracts = electricContracts.Select(x => new ClawbacksRowViewModel
                {
                    EId = x.Contract.EId,
                ...
                contracts.AddRange(gasContracts.Select(x => new ClawbacksRowViewModel
                {
                    EId = x.Contract.EId,
                    BusinessName = x.Contract.BusinessName,
                    MPAN = null,
            ...
            {
                Logger.Log("GetClawbackContracts: " + ex);
