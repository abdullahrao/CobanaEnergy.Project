$(document).ready(async function () {
    ...
    function loadEacLogs() {
        if (!eid) return;
        $.get(`/CrownContract/GetEacLogs?eid=${eid}&contractType=${$("#contractType").val()}`, function (res) {
    ...
    $("#exportInvoiceLogsBtn").on("click", function () {
        $.get(`/CrownContract/GetEacLogs?eid=${eid}&contractType=${$("#contractType").val()}`, function (res) {
            ...
            XLSX.writeFile(wb, "Crown_InvoiceLogs.xlsx");
        })
            .fail(function (xhr) {
    ...
    $("#editCrownContractForm").on("submit", async function (e) {
        e.preventDefault();
        const $btn = $(this).find('button[type="submit"]');
        ...
        try {
            ...
            $.ajax({
                url: '/CrownContract/UpdateContract',
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(payload),
                success: function (res) {
