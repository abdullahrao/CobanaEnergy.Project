$(document).ready(async function () {
    ...
    $("#eacLogForm").on("submit", function (e) {
        ...
        $.ajax({
            url: '/ScottishPowerContract/SaveEacLog',
            type: 'POST',
    ...
    function loadEacLogs() {
        if (!eid) return;
        $.get(`/ScottishPowerContract/GetEacLogs?eid=${eid}&contractType=${$("#contractType").val()}`, function (res) {
    ...
    function renderEacLogs(logs) {
        ...
        <div class="log-entry">
            ...
            <div class="log-field"><span class="log-label">Supplier EAC:</span> ${Number(log.SupplierEac).toFixed(2)}</div>
    ...
    $("#exportInvoiceLogsBtn").on("click", function () {
        $.get(`/ScottishPowerContract/GetEacLogs?eid=${eid}&contractType=${$("#contractType").val()}`, function (res) {
            ...
            const data = [
                ["Year", "EAC Value", "Supplier AVG. EAC","Supplier EAC", "Invoice No", "Invoice Date", "Payment Date", "Invoice (£)", "Supplier EAC D19", "MPAN", "MPRN"],
                ...exportData.map(log => [
                    log.EacYear, log.EacValue, log.FinalEac, log.SupplierEac, log.InvoiceNo, log.InvoiceDate,
    ...
    $("#editScottishPowerContractForm").on("submit", async function (e) {
        e.preventDefault();
        const $btn = $(this).find('button[type="submit"]');
        ...
        try {
            ...
            $.ajax({
                url: '/ScottishPowerContract/UpdateContract',
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(payload),
                success: function (res) {
